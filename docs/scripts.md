...
| Script                   | Purpose                                                                              |
| ------------------------ | ------------------------------------------------------------------------------------ |
| `fm-session-start.sh`    | Compose lock, bootstrap, and wake drain into the single ordered session-start digest, lean by default |
...
| `fm-worktree-lib.sh`     | Shared leased-worktree identity helpers for fm-spawn's pane-path worktree detection  |
...
| `fm-wake-drain.sh`       | Atomically drain queued watcher wakes, print the wake brief by default, then assert watcher liveness |
| `fm-wake-brief.sh`       | Render drained wake records as one compact actionable line each (`FM_WAKE_BRIEF=0` opts out) |
| `fm-wake-lib.sh`         | Shared durable wake queue, portable locks, and watcher identity/health helpers       |
| `fm-classify-lib.sh`     | Shared captain-relevant, declared-external-wait, and idle-absorb classification vocabulary |
